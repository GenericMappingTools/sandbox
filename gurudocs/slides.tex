...
\begin{equation}
\left ( r_2 + \Delta r \bar{u}_s^u \right ) u_0 \left [ (1 + u_0) \log \left (\frac{1 + u_0}{u_0} \right ) - 1 \right ] = \frac{1}{2\Delta h} \left [\frac{V_f - \phi V_0}{\pi \Delta r} - h_1(r_1 + r_2) \right ].
\end{equation}
...
\begin{figure}[h!]
  ...
  \caption{A range of azimuthal variation in slide height can be achieved via the modulating power parameter, $p$.}
  \label{slides_fig3}
\end{figure}
...
\begin{equation}
\bar{s} = 2\int_0^1  \gamma^p d\gamma = \frac{2}{p+1},
\end{equation}
since $p \ge 2$. Now, $1 - \bar{s}$ can be used to evaluate the actual slide volume, i.e., we must adjust the previously obtained upper slide volume:
\begin{equation}
